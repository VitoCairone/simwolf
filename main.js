...
function deqCurrentDelayed() {
  while (delayedRxnArr.length && delayedRxnArr.at(-1)
    .end <= g_now)
...
function findCollidingPairs(creatures) {
  ...
  for (let i = 0; i < creatures.length; i++) {
    ...
    for (const tile of tiles) {
      if (!tileMap.has(tile)) {
        const [tileX, tileY] = tile.split("_")
          .map(Number);
...
window.addEventListener('resize', () => {
  viewportWidth = document.getElementById('field-wrapper')
    .clientWidth;
  viewportHeight = document.getElementById('field-wrapper')
    .clientHeight;
  animat?.updateCamera();
});
